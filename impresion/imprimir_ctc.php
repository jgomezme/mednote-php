...
require ('../medicamentos/funciones/impresion.php');
...
$ordenes=mysql_query("SELECT *, recetas_no_pos.estado FROM recetas_no_pos, medicamentos WHERE id_turno='$id_turno' AND recetas_no_pos.id_medicamento = medicamentos.id_medicamento",$link);
...
$nuevo_select .= "<li title='".$row['observaciones']."'><input type='checkbox' id='orden[".$row['id_receta_no_pos']."]' name='orden[".$row['id_receta_no_pos']."]' onchange= \"xajax_impresion_ctc(xajax.getFormValues('imprimir_ordenes')); \" > ".$row['medicamento_nombre']." ".$row['concentracion_forma']." ";

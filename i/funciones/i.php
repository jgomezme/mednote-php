...
foreach ($titulo[fila] as $fila => $item){ ///para cada fila
		    if($titulo[cups][$item] !=''){
		        $valor_campo[$item] .= "INSERT INTO `autorizaciones_procedimientos_recibidos` 
                        (`id_empresa`,`CUPS`,`cantidad`,`control`,`id_funcionario`)
                        VALUES (";
...
//////GRABA LA SOLICITUD DE AUTORIZACION  
  ...
  $valores_consulta =  substr ($valores_consulta, 0, -1);
    ...
    $consulta ="INSERT INTO  autorizaciones_solicitud ( $campos_consulta ) 
						    ...
						    $grabar = mysql_query($consulta,$link);
//------------------------------------------------------- modificacion ---------------------------------------------
$id_autorizacion_solicitud_resultset = mysql_query("SELECT id_autorizacion_solicitud FROM autorizaciones_solicitud WHERE timestamp_autorizacion = '$timestamp'",$link);
$id_autorizacion_solicitud_final = 0;
while( $resultset = mysql_fetch_array( $id_autorizacion_solicitud_resultset ) ) {
    $id_autorizacion_solicitud_final = $resultset['id_autorizacion_solicitud'];
}
//------------------------------------------------------------------------------------------------------------------
foreach ($titulo[fila] as $fila => $item){ ///para cada fila
				if($titulo[cups][$item] !=''){
					...
					$valor_campo[$item] .= "','".$titulo["control"];
					$valor_campo[$item] .= "','$_SESSION[id_usuario]";
                    $valor_campo[$item] .= "','$id_autorizacion_solicitud_final";
					$valor_campo[$item] .= "')";//$id_autorizacion_solicitud_final	
											}	
											}
...
<div align='center' ><h2>Solicitud de autorización</h2>".usuario_datos_consultar($id,'usuario','nombre_completo')."
				<table cellpadding='10' cellspacing='0' border='1' align='center' width='100%'  style='background-color: #E6F6FA; border-color: #1E90FF; border-width: 1px; width: 80%; '>
					<tr>
						<td><b>Consulta de referencia:</b> ".usuario_datos_consultar($id,'consultas_referencia','id_atencion_inicial')."
						</td>
						<td><a href='adentro.php?page=modulos&modulo=autorizacion&usuario=".$_GET['usuario']."' >Agregar datos manualmente</a></td>
					</tr>
					<tr valign='top'>
...
if ($tipo =='sa'){$tabla='autorizaciones_solicitud'; $consulta ="SELECT * FROM $tabla WHERE 1 $usuario AND estado!=3 order by `id_autorizacion_solicitud` desc LIMIT $limite ";}
...
if ($tipo == 'sa'){///inicia solicitud de autorizacion
...
$resultado .= "<tr bgcolor='#c2f0e5'>
						...
						<td title='Ubicacion' ><b>&nbsp;Ubicación&nbsp;</b></td>
						<td title='PIN del formulario' ><b>&nbsp;PIN&nbsp;</b></td>
						<td><strong><div  title='Opciones de envio o formato'>&nbsp;Acción&nbsp;</div></strong></td>
						<td><strong><div  title='Estado de la autorización'>&nbsp;Estado&nbsp;</div></strong></td>
				</tr>";
while( $row = mysql_fetch_array( $sql ) ) {

$direccion=usuario_datos_consultar($row[id_cliente],cliente,'email_autorizaciones');
if($direccion !=''){
                    $enviar = "<a onClick=\"xajax_envio_documentos('$row[id_autorizacion_solicitud]','autorizaciones_solicitud','$row[control]','$row[id_cliente]','clientes','1','cuerpo_del_mensaje','capa_sa_$row[control]', $row[id_autorizacion_solicitud])\">
...
$fila = $fila +1;
if ($fila %2 == 0){
                   $clase = "par";$bg="#EFFBFB";}else{$clase = "impar"; $bg="#FFFFFF";
                   }
$estate = "vacio";
$titu = "vacio";                  
if($row[estado]=="1" || $row[estado]=="0"){
	$estate = "<a href='#' onclick='cambiarEstadoSolicitudAutorizacion(\"enviar\" , \"$row[id_autorizacion_solicitud]\"); return false;'>Enviar</a>";
	$titu = "Clic para confirmar que se envio la solicitud";	
}else if ($row[estado]=="2"){
	$estate = "<a href='#' onclick='cambiarEstadoSolicitudAutorizacion(\"confirmar\" , \"$row[id_autorizacion_solicitud]\");return false;'>Autorizar</a>";
	$titu = "Clic para afirmar que se autorizo la solicitud";	
}                   
$resultado .= "<tr onMouseOver=\"uno(this,'#c2f0e5');\" onMouseOut=\"dos(this,'$bg');\" bgcolor='$bg' valign='top'>
				<td title=''><b>".usuario_datos_consultar($row[id_usuario],'usuario','documento_numero')."</b> ".usuario_datos_consultar($row[id_usuario],'usuario','nombre_completo')."
				<a href='adentro.php?page=suscriptores&usuario=$row[id_usuario]'><img src='images/buscar.gif' border='0' Alt='[--O]' title='Buscar este usuario [--O]'></a></td>
				...
				<td title=''>".date('Y-m-d H:i',$row[timestamp_autorizacion])."</td>
				<td title='Ubicación del paciente' >&nbsp;&nbsp; $row[ubicacion_servicio] $row[ubicacion_cama]</td>
				<td title='Clave para acceder al formulario' align='center' >$row[pin]</td>
				<td >
				<div id='capa_sa_$row[control]'>
							
				$enviar
				".envio_revisar($row[control],'','')."<a href='$url_aplicacion/i/sa.php?r=$row[control]&id_solicitud=$row[id_autorizacion_solicitud]' target='_blank' title='Informe de la Atención inicial de urgencias'>
				</div><font size='-2' title='Muestra este formato'>VER FORMATO</font></a></td>
				<td title='$titu' align='center' ><div id='tdautorizacion$row[id_autorizacion_solicitud]'>$estate</div></td>
				</tr>";
		} //cierra el while
}//// fin solicitud de informacion cierra el if						

...
/// fuunciones contabilidad
//include_once("contabilidad/funciones/contabilidad.php");
/// funciones autorizaciones
include_once("i/funciones/i.php");

$xajax->registerFunction("odontograma_procedimientos_listado");
$xajax->registerFunction("odontologia_cotizacion_grabar");
...
$campos_formulario .="<br><b>$titulo</b><br> <input type='text' id='buscar$item' name='buscar$item' value='$titulo ' size='70'
 								onkeyup=\"if(revisa('buscar$item')=='si'){numeros(event,'$item','generico','$tabla','$campo','$campo_descripcion'); }
                else{limpia($item)}\"  onClick=\"this.value='' \"/>
                <input type='$tipo' id='$item' name='$item' value='' size='5'
 								 onClick=\"this.value='' \"/>
...
width:300px;
       ...
       -->
                ...
                <div id='contenedor$item' onmouseover='sobre()' style='height:200px;  display:inline' ></div>
                ";
...
return $campos_formulario;
 ...
 $link=Conectarse(); 
//$tabla="cie_10";
if($tabla == 'd9_users'){$andX ="OR documento_numero  LIKE '%%$valor%%'"; $campo_extra =', documento_numero AS campo_extra';
...
if($tabla == "d9_users"){
 $pinta .= "<li title ='OPRIMA PARA SELECCIONAR ESTE USUARIO' onclick=\"window.location='adentro.php?page=suscriptores&usuario=$row[campo]';
 document.getElementById('contenedor$item').style.display='none'\"' class='cursor' id='li$row[campo]' ><b>$row[campo_extra]</b> 
 $r

 </li>";
 						}else{
$pinta .= "<li title ='OPRIMA PARA SELECCIONAR ESTE USUARIO' onclick=\"document.getElementById('buscar$item').value='$row[campo_descripcion]' ;document.getElementById('$item').value='$row[campo]' ;
 document.getElementById('contenedor$item').style.display='none'\"' class='cursor' id='li$row[campo]' ><b>$row[campo_extra]</b> 
 $r
...
function envio_documentos($id_documento,$tabla,$referencia,$id_destino,$destino_tabla,$id_canal,$cuerpo,$capa, $id_solicitud){
include('includes/datos.php');
$debug = " IMULACRO ";
$debug_2 = " <img src='$url_aplicacion/images/atencion.gif' ALT='Notificacion'> Esta es una notificacion envia de forma automatica por nuestro sistema de información <a href='http://opuslibertati.org' >http://opuslibertati.org</a> con el fin de cumplir la normatividad vigente Decreto 4747<hr>";
$respuesta = new xajaxResponse('utf-8');
;
...
$formato_url=$url_aplicacion."/i/sa.php?r=$referencia&id_solicitud=$id_solicitud";
//$formato_url=$url_aplicacion."/i/sa.php?r=";
$formato_pin=usuario_datos_consultar($id_documento,$tabla,'pin');														
...
<table cellpadding='0' cellspacing='10' border='0' align='center' bgcolor='#FFFFFF' width='500px'>
	<tr>
		...
		<div align='center'><img src='$url_aplicacion/images/email_pie.gif' alt ='Un proyecto de: http://OpusLibertati.org - Powered by: http://GaleNUx.com' title='Un proyecto de: http://OpusLibertati.org - Powered by: http://GaleNUx.com'></div>
...
$headers .= 'From: Autorizaciones tu entidad <autorizaciones@tuentidad.com>'. "\r\n";
$headers .= 'Cc: otracuenta@otroservidor.com' . "\r\n";

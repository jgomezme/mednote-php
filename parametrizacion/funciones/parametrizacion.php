...
Nombre del medicamento:<br><input type='text' name='medicamento_nombre' size='32' maxlength='32' ><br>
...
function importar($formulario,$capa,$tabla,$path){
//creo el xajaxResponse para generar una salida
...
if (strlen($archivo)>3 )
							{$path2="$path$archivo";
							$nuevo_select .= "<li>$archivo <a onClick=\"xajax_importar('preview','capa_subir_contratos','$tabla','$path2')\" class='cursor'> [<font size='-3' COLOR='green' title=''>IMPORTAR</font></a>]</li>";
							}
  									}
$directorio->close();
									}////fin de listado
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
    ...
    //for ($c=0; $c < $num; $c++) {
    $control = md5($data[1]);
    if($data[15]=='1') {$genero="M";}
    if($data[15]=='2') {$genero="F";}
 ...
 '2','1','$data[8]','$data[7]','$data[6]','$data[4]','$data[5]','$data[2]','$data[3]','$data[4] $data[5] $data[2] $data[3]','$data[14]','$data[0]','$data[1]','$data[17]','$data[11]','$data[9]','$control','$data[19]','$data[22]','$data[18]','COL','$genero','$data[10]','$data[23]')<hr>";
    //}
}
fclose($handle);
...
if ($tipo=='consultar_campos'){
 $consulta="SELECT consulta_tipo_campos.id_campo,campo_nombre, obligatorio,control,prellenado FROM consulta_tipo_campos, consulta_campos WHERE tipo_consulta = $id AND consulta_tipo_campos.id_campo = consulta_campos.id_consulta_campo ORDER BY campo_nombre";
 $sql =mysql_query($consulta,$link);
...
$resultado ="<h2>$consulta_tipo_nombre</h2>
				<table>"; 
				$resultado .= "<tr><td><b>Campo</b></td><td><b>Obligatorio</b></td><td>Prellenado</td><td>X</td></tr>";

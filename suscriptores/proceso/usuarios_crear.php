...
$consulta_recien_ingresado=mysql_query('SELECT id, entkey, username FROM d9_users WHERE username=\''.$username.'\' AND p_nombre=\''.$p_nombre.'\' ',$link);
...
mysql_query('UPDATE d9_users SET entkey=\''.$entkey.'\' WHERE id=\''.$id.'\'');
...
include_once("../../librerias/conex.php");
$link=Conectarse(); 
$consulta_registro_ingresado=mysql_query('SELECT id, entkey, username FROM d9_users WHERE id=\''.$id.'\'',$link);

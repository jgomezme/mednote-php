...
FROM `d9_users` 
WHERE (nombre_completo LIKE $suscriptor_mayus) OR  (nombre_completo LIKE $suscriptor_minus) OR (documento_numero LIKE $suscriptor) $w_grupo ORDER BY nombre_completo  LIMIT 100",$link);
echo mysql_error();
...
<?
	...
	echo "<tr  onMouseOver=uno(this,'red'); onMouseOut=dos(this,'FFFFFF'); bgcolor=#FFFFFF ><td></td><td onMouseOver=uno(this,'#f5f3d7'); onMouseOut=dos(this,'FFFFFF'); bgcolor=#FFFFFF><a href=\"javascript:retornar_ID('$row[0]')\"  onsubmit=SoloCerrar(); >".$row[1]." ".$row[2]." ".$row[3]." ".$row[4]."</a></td><td></td></tr>";
	}
?>
	</table>
	</body>

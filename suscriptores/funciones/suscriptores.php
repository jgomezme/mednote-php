...
$Productos=mysql_query("SELECT * FROM d9_users , sucursales_ocupacion, sucursales
								WHERE id_grupo = 3 
								$w 
								AND ping >= '$diferencia' 
								AND d9_users.id = sucursales_ocupacion.id_usuario 
...
//if($id==''){
$consulta_documento = mysql_query ("SELECT documento_numero FROM d9_users WHERE documento_numero = '$documento_numero'",$link);
...
$asunto ="[Opus Libertati $cliente] Bienvenido al sistema";
$cuerpo ="
<html>
<body>
...
$ultimo_usuario = mysql_query ("SELECT id FROM d9_users WHERE control = '$control' ",$link);
...
$consulta_usuario = mysql_query ("SELECT id FROM d9_users WHERE id = '$id' AND id_empresa='$id_empresa' ",$link);
...
//$usuario_grabado .= mysql_result($consulta_usuario,0,"id"); 
  mysql_query("UPDATE `d9_users` SET 
`id_grupo`='$id_grupo',  
...
`responsable_telefono`='$responsable_telefono'
 WHERE `d9_users`.`id` = $id LIMIT 1",$link);  
...
$capa = "formulario";
$Usuario_Datos=mysql_query("SELECT * FROM d9_users  WHERE  (id =  '$id')",$link); 
...
$Usuarios=mysql_query("SELECT * FROM d9_users WHERE id_grupo ='2' AND id_cliente = '$id_cliente'",$link);  
$usuarios .= "
